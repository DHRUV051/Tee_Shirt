...
const Header = () => {
    ...
    return(
        <>
            ...
            <div className="bg-[rgba(248,248,248,1)] md:hidden items-center 
            h-[50px] px-16 w-auto">
            ...
            <div className=" ml-auto flex justify-between gap-4 items-center font-semibold text-[18px] text-[rgba(0,0,0,1)]">🔥Free shipping on all U.S. orders $50+</div>

            <div className=" ml-auto flex justify-between gap-4 items-center text-[rgba(155,155,155,1)]"> 

                    <h1 className=" text-[rgba(0,0,0,1)] text-[15px] font-semibold">Login</h1>
                    <button className='bg-[rgba(0,0,0,1)] px-3 py-1 rounded-[7px] '>
            <Link href='/Home' className='text-white text-center flex'>
              <h1 className='font-bold text-[15px]'>Sign Up</h1> 
            </Link>
          </button>
            </div>
        </div>
      ...
      <Image src={div}  height={55} width={400} alt="call"  />  
        ...
        <div className="justify-between font-bold md:ml-36 text-[16px]">🔥Extra <span className=' text-green-600'>Sale 30% off</span></div>
         ...
         {toggle ? (
          <IoMdClose onClick={()=>setToggle(!toggle)} size={30} className=' flex lg:hidden sm:px-3 sm:w-12 sm:h-6 md:px-3 md:w-12 md:h-6' />   
          ) : (
          <IoMdMenu  onClick={()=>setToggle(!toggle)} size={30} className=' block lg:hidden sm:px-3 sm:w-12 sm:h-6 md:px-3 md:w-12 md:h-6' />
          )}
